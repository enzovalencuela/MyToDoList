...
import {
  GripVertical, ChevronDown, ChevronUp, Trash2, Pencil, Check, X,
...
function getDueDateStatus(dueDate?: string | null) {
  if (!dueDate) return { label: "Indefinido", color: "bg-gray-400 text-white" };
  const now = new Date();
  ...
  if (diffDays <= 3) return { label: "Próxima", color: "bg-yellow-500 text-white" };
  return {
    ...
    >
      ...
      <div className="flex items-center gap-2 px-4 py-3">
        ...
        <button
          ...
          className="text-[var(--subText)] hover:text-[var(--text)]"
        >
